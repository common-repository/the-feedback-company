<?php
	...
	class feedbackcompany_api
	{
		...
		/**
		 * Function for clearing oath authentication cache so a new token is requested
		 *
		...
		function oauth_refreshtoken()
		{
			...
			error_log(var_export($lastfailedcall, true));
			if ($lastfailedcall !== false && time() < (int)$lastfailedcall + 1800)
				return;
